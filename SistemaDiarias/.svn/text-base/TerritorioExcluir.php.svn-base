...
<!--
	...
	function ExcluirForm(frm, checkbox)
	 {
		frm.action="TerritorioExcluir.php?excluirMultiplo=1&acao=excluir";
		frm.submit();
	 }
...
<?
										...
										If ($ExcluirCheckbox == 1)
                                        {
											...
											$sqlConsulta = "SELECT * FROM diaria.territorio WHERE territorio_cd IN ('".$codCheckbox."') ORDER BY UPPER(territorio_ds)";
