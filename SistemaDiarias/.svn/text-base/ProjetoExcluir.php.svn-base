...
<!--
	...
	function ExcluirForm(frm, checkbox)
	 {
		frm.action="ProjetoExcluir.php?excluirMultiplo=1&acao=excluir";
		frm.submit();
	 }
...
<?
                                        ...
                                        {
											$sqlConsulta = "SELECT * FROM diaria.projeto WHERE projeto_cd IN ('".$codCheckbox. "') ORDER BY UPPER(projeto_ds)";
											$rsConsulta = pg_query(abreConexao(),$sqlConsulta);

                                            $Codigo = $codCheckbox;
                                            while($linharsConsulta=pg_fetch_assoc($rsConsulta))
                                            {  	$Numero	  	  = $linharsConsulta['projeto_cd'];
                                                $Descricao	  = $linharsConsulta['projeto_ds'];
												$StatusNumero  = $linharsConsulta['projeto_st'];
