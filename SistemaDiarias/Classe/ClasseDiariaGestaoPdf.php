...
ElseIf ($AcaoSistema == "imprimir")
{
	...
	$Valor                  = 'R$ '.number_format($Valor, 2, ',', '.');
        ...
        if($qtdeRoteiros > 0)
        {
            $sqlRoteiromultiplo = "SELECT * FROM diaria.dados_roteiro_multiplo WHERE diaria_id = ".$Codigo." AND dados_roteiro_status = 0";

            $rsRoteiroMultiplo = pg_query(abreConexao(),$sqlRoteiromultiplo);    

            while($linharsRoteiroMultipo = pg_fetch_assoc($rsRoteiroMultiplo))
            {                   
                $dataPartida[$linharsRoteiroMultipo['controle_roteiro']]      = $linharsRoteiroMultipo['diaria_dt_saida'];
                $horaPartida[$linharsRoteiroMultipo['controle_roteiro']]      = $linharsRoteiroMultipo['diaria_hr_saida'];
                $dataChegada[$linharsRoteiroMultipo['controle_roteiro']]      = $linharsRoteiroMultipo['diaria_dt_chegada'];
                $horaChegada[$linharsRoteiroMultipo['controle_roteiro']]      = $linharsRoteiroMultipo['diaria_hr_chegada'];
                $diaSemanaPartida[$linharsRoteiroMultipo['controle_roteiro']] = diasemana($linharsRoteiroMultipo['diaria_dt_saida']);
                $diaSemanaChegada[$linharsRoteiroMultipo['controle_roteiro']] = diasemana($linharsRoteiroMultipo['diaria_dt_chegada']);
                $qtde[$linharsRoteiroMultipo['controle_roteiro']]             = $linharsRoteiroMultipo['diaria_qtde'];
                $valor[$linharsRoteiroMultipo['controle_roteiro']]            = $linharsRoteiroMultipo['diaria_valor'];
                
                if($linhaConsulta['diaria_desconto'] == 'N')
                {
                    $desconto[$linharsRoteiroMultipo['controle_roteiro']] = 'Não';
                }
                else
                {
                    $desconto[$linharsRoteiroMultipo['controle_roteiro']] = 'Sim';
                }
                
                $sqlRoteiro = "SELECT roteiro_origem, roteiro_destino FROM diaria.roteiro WHERE diaria_id = ".$Codigo." AND controle_roteiro = ".$linharsRoteiroMultipo['controle_roteiro'];
                ...
                $roteiro[$linharsRoteiroMultipo['controle_roteiro']] = $Inicio.$Meio.$Final;
            }
        }
        // *******************************************************************
        ...
        If($linhaConsulta)
        {
            ...
            $impProcessoSt              = $linhaConsulta['imp_processo_st'];
        }
		// *******************************************************************
 		// Consulta do Aprovador. 
        ...
        If ($linharsEndereco)
        {
            if(strlen($linharsEndereco['endereco_ds']) > 40)
            {
                $enderecoDs = substr($linharsEndereco['endereco_ds'], 0, 42);		
                $enderecoDs = trim($enderecoDs)." ...";                
            }
            else
            {
                $enderecoDs = $linharsEndereco['endereco_ds'];
            }
            
            $Endereco = $enderecoDs.", " .$linharsEndereco['endereco_num']. " " .$linharsEndereco['endereco_complemento']. ", " .$linharsEndereco['endereco_bairro']. ", " .$linharsEndereco['municipio_ds']. " - " .$linharsEndereco['estado_uf'];
        }
        Else
        {
            $Endereco = "";
