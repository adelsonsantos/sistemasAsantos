...
//  Possui filtro e atributo
                     $sqlConsulta = "SELECT * FROM diaria.diaria d, dados_unico.funcionario f, dados_unico.pessoa p WHERE (p.pessoa_id = f.pessoa_id) AND (d.diaria_beneficiario = f.pessoa_id) AND diaria_st IN (2,6) AND diaria_agrupada = 0 AND diaria_cancelada = 0 AND (pessoa_nm ILIKE '%" .$RetornoFiltro."%' OR diaria_numero ILIKE '%" .$RetornoFiltro. "%') ORDER BY ".$_GET['atributo']." ASC";
                }
            else
                {
//  Possui filtro, mas n�o possui atributo 
                    $sqlConsulta = "SELECT * FROM diaria.diaria d, dados_unico.funcionario f, dados_unico.pessoa p WHERE (p.pessoa_id = f.pessoa_id) AND (d.diaria_beneficiario = f.pessoa_id) AND diaria_st IN (2,6) AND diaria_agrupada = 0 AND diaria_cancelada = 0 AND (pessoa_nm ILIKE '%" .$RetornoFiltro."%' OR diaria_numero ILIKE '%" .$RetornoFiltro. "%') ORDER BY TO_DATE(diaria_dt_saida,'DD/MM/YYYY') DESC, diaria_hr_saida ASC";
                }
...
// n�o possui atributo e n�o possui filtro
                        $sqlConsulta = "SELECT * FROM diaria.diaria d, dados_unico.funcionario f, dados_unico.pessoa p WHERE (p.pessoa_id = f.pessoa_id) AND (d.diaria_beneficiario = f.pessoa_id) AND diaria_st IN (2,6) AND diaria_cancelada = 0 AND diaria_agrupada = 0 ORDER BY TO_DATE(diaria_dt_saida,'DD/MM/YYYY') DESC, diaria_hr_saida ASC";
                }
	}
    }
    else
...
//  Verifica se foi enviado um atributo
            if ($_GET['atributo']!='')
                {
//  Possui filtro e atributo
                     $sqlConsulta = "SELECT * FROM diaria.diaria d, dados_unico.funcionario f, dados_unico.pessoa p WHERE (p.pessoa_id = f.pessoa_id) AND diaria_st IN (2,6) AND diaria_agrupada = 0 AND diaria_cancelada = 0 AND (pessoa_nm ILIKE '%" .$RetornoFiltro."%' OR diaria_numero ILIKE '%" .$RetornoFiltro. "%') ORDER BY ".$_GET['atributo']." ASC";
                }
            else
                {
//  Possui filtro, mas n�o possui atributo 
                    $sqlConsulta = "SELECT * FROM diaria.diaria d, dados_unico.funcionario f, dados_unico.pessoa p WHERE (p.pessoa_id = f.pessoa_id) AND diaria_st IN (2,6) AND diaria_agrupada = 0 AND diaria_cancelada = 0 AND (pessoa_nm ILIKE '%" .$RetornoFiltro."%' OR diaria_numero ILIKE '%" .$RetornoFiltro. "%') ORDER BY TO_DATE(diaria_dt_saida,'DD/MM/YYYY') DESC, diaria_hr_saida ASC";
                }
...
// n�o possui filtro, mas posssui Atributo.
                ...
                {
// n�o possui atributo e n�o possui filtro
                        $sqlConsulta = "SELECT * FROM diaria.diaria d, dados_unico.funcionario f, dados_unico.pessoa p WHERE (p.pessoa_id = f.pessoa_id) AND diaria_st IN (2,6) AND diaria_cancelada = 0 AND diaria_agrupada = 0 ORDER BY TO_DATE(diaria_dt_saida,'DD/MM/YYYY') DESC, diaria_hr_saida ASC";
                }
	}
    }
...
elseif ($AcaoSistema == "empenhar"){
	...
	if( $Diaria_Agrupada == 0){	// Diarias N�o Agrupadas - N�o Tem SUPER SD
		if( $processo_diaria == ""){
	...
	}else{ // Diarias Agrupadas - Tem SUPER SD
		$DTAno        = date("Y");
 ...
 ElseIf ($AcaoSistema == "empenharST") // LIBERAR EMPENHO
{
	...
	if($Diaria_Agrupada == 0 ){ // N�o � Di�ria Agrupada		
		$sqlAltera = "UPDATE diaria.diaria SET  diaria_st = 3,
                             diaria_devolvida = 0
                             WHERE diaria_id = ".$Codigo;
	}else{ // Di�rias Agrupadas
		$sqlAltera = "UPDATE diaria.diaria SET  diaria_st = 3,
                             diaria_devolvida = 0
                             WHERE super_sd = '$Super_SD'";
	}
