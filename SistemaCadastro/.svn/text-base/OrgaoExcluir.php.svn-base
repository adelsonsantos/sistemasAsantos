...
<!--
	...
	function ExcluirForm(frm, checkbox)
	 {
		frm.action="OrgaoExcluir.php?excluirMultiplo=1&acao=excluir";
		frm.submit();
	 }
...
<?
										...
										If ($ExcluirCheckbox == 1)
                                        {
											...
											$sqlConsulta = "SELECT * FROM dados_unico.orgao WHERE orgao_id IN (" .$codCheckbox. ") ORDER BY UPPER(orgao_ds)";
                                            $rsConsulta = pg_query(abreConexao(),$sqlConsulta);



											$OrgaoCodigo = $codCheckbox;
...
?>
                                                        <tr height="21" class="dataField">
                                                            <td>&nbsp;<?=$OrgaoDescricao?></td>
                                                            <td align="center"><?=$OrgaoStatus?></td>
                                                        </tr>
<?                                           }
                                        }
...
<?
                                    ...
                                    {
?>
                                    	<button style="width:70px" onClick="Javascript:window.location.href='<?=$PaginaLocal?>Inicio.php?acao=excluir&cod=<?=$OrgaoCodigo?>';" class="botao">Sim</button>&nbsp;&nbsp;
<?
                                    }
?>
...
<input name="txtCodigo" type="hidden" value="<?=$OrgaoCodigo?>">
</form>
