...
$sqlConsulta = "SELECT DISTINCT d.diaria_numero, d.projeto_cd, d.acao_cd, d.territorio_cd, d.fonte_cd,d.diaria_valor,
                       d.diaria_processo, d.diaria_qtde, d.diaria_valor_ref, d.diaria_unidade_custo, d.diaria_st,
                       d.diaria_dt_saida, d.diaria_empenho, d.diaria_descricao, d.convenio_id, d.indenizacao, d.ger_id,
                       pf.pessoa_fisica_cpf, ft.funcionario_tipo_ds, p.pessoa_nm, f.funcionario_matricula, f.pessoa_id,
                       EF.est_organizacional_id, ESTO.est_organizacional_unidade_executora,
                       ESTO.est_organizacional_centro_custo_num
                    FROM diaria.diaria d, diaria.diaria_motivo dm, diaria.projeto dp, dados_unico.funcionario f,
                         dados_unico.funcionario_tipo ft, dados_unico.pessoa p, dados_unico.pessoa_fisica pf,
                         dados_unico.est_organizacional_funcionario EF, dados_unico.est_organizacional ESTO
                    WHERE est_organizacional_funcionario_st = 0
                        ...
                        AND (d.diaria_st = 2 OR d.diaria_st = 6)
                        AND (d.projeto_cd = dp.projeto_cd AND dp.est_organizacional_id = ESTO.est_organizacional_id)
                        AND d.diaria_id in (".$ImpressaoMultipla.")";
                        //AND d.diaria_unidade_custo = ESTO.est_organizacional_id";
...
while ($linhars = pg_fetch_assoc($rsConsulta))
    {
        ...
        $pdf->Cell (35,5,substr($DiariaProcessoNum, 6, 7),1,0);
        ...
        $pdf->SetFont ("Times", "B",9);
        $pdf->Cell(190,6,"MOTIVO DA VIAGEM : ",1,1 );
        $pdf->SetFont ("Times", "",8);
